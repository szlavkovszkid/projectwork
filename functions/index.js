...
const serviceAccount = require('./sapsystem-e70ad-firebase-adminsdk-ikm2m-9bf8542746.json');
admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
  databaseURL: "https://sapsystem-e70ad-default-rtdb.europe-west1.firebasedatabase.app"
});
...
app.post('/setPhotoURL', verifyToken, (req, res) => {
  ...
  // Ellenőrizd, hogy csak a saját photoURL-jét próbálja-e módosítani a felhasználó
  if (req.user.uid !== uid) {
    res.status(403).json({ error: 'Nincs jogosultság a módosításra.' });
    return;
  }

  // Felhasználó photoURL-jének módosítása
  admin.auth().updateUser(uid, { photoURL })
    .then(() => {
      console.log('Felhasználó photoURL-je sikeresen frissítve.');
      res.json({ message: 'OK' });
    })
    .catch((error) => {
      console.error('Hiba történt a felhasználó photoURL-jének frissítésekor:', error);
      res.status(500).json({ error: 'A photoURL frissítése sikertelen.' });
    });
});
